---
title: "Pre_Reg-Chinese Characters"
...
## Data analysis for pre-registration
...
lmer(diff ~ type (effect coded) * val (effect coded) + condition (effect coded)* type (dummy coded) * val(dummy coded) +  (type(effect coded) * (effect coded)|subject))

```{r, echo = TRUE, include = TRUE}
#model2
lmer1_2 <- lmer(response ~ val_effect*type_effect + condition_effect:val:type_specific
                + (val_effect*type_effect|subject),
                direct, REML = FALSE)
