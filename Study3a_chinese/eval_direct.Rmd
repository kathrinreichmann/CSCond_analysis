...
Explanation: similarity to "category-defining feature" (many) is greater for GSs than similarity to "whole stimulus" (one) -> wider generalization can be expected
...
--> Do we want this "distractor" level? or do we skip this?
...
```{r use model 3 to plot individual slopes, echo = FALSE, include = TRUE}

indPlot3 <- ggplot(indPlotData[!indPlotData$condition == "one",], aes(x = type_discrete, y = diff, group = subject, color = condition, shape = condition)) +
  facet_wrap(.~ subject, nrow = 5) +
  geom_point(show.legend = TRUE, alpha = .6) +
  geom_smooth(method = "lmer", alpha = .6, se = FALSE) +  
  scale_color_brewer(palette = "Paired") 
indPlot3
...
+ **ConditionMany [Gruppeneffekt]:**  On average, the difference score is reduced by XX points for non-predictive in the „one“ condition in comparison to non-predictive in the „many“ condition. -> not interesting

+ **Type_discretePredictive [Component]:** In the „one“ condition, the mean difference score is reduced by XX points for predictive components, in comparison to non-predictive components -> **hypothesis 2a: not significant**

+ **Type_discretePredictive:conditionMany:** Average decrease of difference scores from „non-predictive“ to „predictive“ by XX points for „many“ condition beyond the decrease of difference scores from „non-predictive“ to „predictive“ for „one“ condition. -> **hypothesis 2b: significant**
