...
test <-aggregate(diff ~ type_specific*condition, directDiff, mean)
mean(test$diff) #intercept
aggregate(diff ~ type_specific, test, mean) #main effect type
aggregate(diff ~ condition, test, mean) #main effect condition
#interaction effect
int1 <- aggregate(diff ~ type_specific, directDiff[directDiff$condition == "one",], mean)
int2 <- aggregate(diff ~ type_specific, directDiff[directDiff$condition == "many",], mean)
mean(int1$diff)
mean(int2$diff)

lmer1Dum <- lmer(diff ~ condition*type_specific 
                  + (1|subject), 
                  directDiff, REML = FALSE)
summary(lmer1Dum)
